...
interface HermanosApi {
    ...
    suspend fun todayServices(@Query("date") date: String): AppointmentsResponse

    @GET("mobile_api.php?action=calendar")
    suspend fun calendar(@Query("start") start: String, @Query("end") end: String): AppointmentsResponse

    @GET("mobile_api.php?action=appointment_detail")
    suspend fun appointmentDetail(@Query("id") id: Long): AppointmentDetailResponse
    ...
    @FormUrlEncoded
    @POST("mobile_api.php?action=appointment_update")
    suspend fun updateAppointment(
        @Field("appointment_id") appointmentId: Long,
        @Field("client_phone") phone: String,
        @Field("client_country_code") countryCode: String,
        @Field("client_name") clientName: String,
        @Field("service_description") serviceDescription: String,
    ...
    suspend fun messages(@Query("jid") jid: String, @Query("limit") limit: Int = 50): MessagesResponse

    @FormUrlEncoded
    @POST("mobile_api.php?action=crm_send")
    suspend fun sendMessage(@Field("jid") jid: String, @Field("message") message: String): BasicResponse

    @FormUrlEncoded
    @POST("mobile_api.php?action=crm_analyze")
    suspend fun analyze(@Field("jid") jid: String): AnalyzeResponse

    @FormUrlEncoded
    @POST("mobile_api.php?action=crm_mark_read")
    suspend fun markRead(@Field("jid") jid: String): BasicResponse
}
